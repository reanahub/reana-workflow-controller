...
def umount(aliases):
    """Unmounts all paths recorded in /etc/active_mounts.txt."""

    print("Starting unmount sequence...")
    try:
        for i in range(len(aliases)):
            target_path = os.path.join("/s3-data", aliases[i][0])
            target_path = os.path.join(target_path, aliases[i][1])
            print(f"Unmounting: {target_path}")
            rc = os.system(f"fusermount -u {target_path}")
            if rc != 0:
                print(f"Warning: Failed to unmount {target_path}. It might be busy.")
            else:
                print(f"Successfully unmounted {target_path}")
