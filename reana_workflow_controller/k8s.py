...
class InteractiveDeploymentK8sBuilder(object):
    ...
    def add_user_secrets(self, secret_names=None):
        """Mount the "file" secrets and set the "env" secrets in the container."""
        user_secrets = UserSecretsStore.fetch(self.owner_id).get_scoped_secrets(
            secret_names
        )
        if not user_secrets.get_secrets():
            return

        if user_secrets.has_file_secrets():
    ...
    def get_deployment_objects(self):
        """Return the already built Kubernetes objects."""
...
def build_interactive_jupyter_deployment_k8s_objects(
    ...
    :param secret_names: Explicit allowlist of user secret names to expose
        to the interactive session. If unset, all user secrets remain
        available.
    """
    ...
    if expose_secrets:
        deployment_builder.add_user_secrets(secret_names=secret_names)
